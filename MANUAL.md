...
F4 / D11 をミキサーや後段モジュールへ接続します。出力は Arduino の 5V 矩形波です。ライン入力や外部機材へ入れる場合は、必要に応じて音量を下げてください。F1、F2、F3 に外部信号を入れる場合は 0V から 5V の範囲（正の電圧のみ）にしてください。ユーロラックなどのマイナス電圧（負電圧）を含む信号や、5Vを超える高い電圧を直接入れると回路が破損する原因になります。必ず電圧を適切な範囲に調整してから接続してください。
