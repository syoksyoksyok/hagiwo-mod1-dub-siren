...
単位は直接Hzとして扱われ、最終的に `baseFreq + lfoValue` の形で周波数に加算されます。この値はLEDの最大輝度にも使われ、深さが大きいほど点灯時のLEDが明るくなります。
...
10. `lfoValue > 0` のときLEDのPWM輝度をLFO深さに応じた値へ設定し、`lfoValue <= 0` のとき消灯する。
...
D3のハードウェアPWMはTimer2系で、`tone()` と衝突する可能性があります。そのためLEDの明るさ制御は `analogWrite()` ではなく、`micros()` を使ったソフトウェアPWMで行います。
