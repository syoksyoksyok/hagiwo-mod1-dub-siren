...
`D11` が音声出力向きなのは、現行スケッチが `tone(PinConfig::SPEAKER, freq)` で矩形波を出すためです。`tone()` はデジタルピンをHIGH/LOWに切り替えて周波数信号を作るので、アナログ入力ピンではなくデジタル出力ピンに接続されたジャックが適しています。`F4` は `D11` の単独ネットとして扱われ、`F2` / `F3` のように別のアナログ入力ピンと共有されていないため、音声出力専用として使いやすい構成です。

ただし、`tone()` の出力はDAC音声ではなく5V系の矩形波です。音量制御もできません。`F4` 周辺には `R8 1k`、保護ダイオード、`C10 1uF` があり、ジャック接続時の保護や高周波ノイズ低減に寄与しますが、一般的なラインレベル音声出力そのものではありません。外部ミキサーやモジュラー機器へ接続する場合は、受け側の入力レベルと必要に応じた減衰を考慮します。
...
`A3 / D17` をデジタル入力として読みます。GATE入力がHIGHの間だけD11へ発音し、LOWの間は `noTone()` で停止します。LOWからHIGHへ変化したときは `state.angle` を `0.0` に戻し、LFO位相を先頭から再開します。
...
`tone()` は音量制御を持たないため、このスケッチでは音量フェードや実振幅制御は行っていません。
D3のハードウェアPWMはTimer2系で、`tone()` と衝突する可能性があります。そのためLEDの明るさ制御は `analogWrite()` ではなく、`micros()` を使ったソフトウェアPWMで行います。LED輝度はLFO値を0-最大輝度へ連続変換するため、A2を左端にしてLFOが遅い場合もゆっくりした明暗変化として確認できます。
...
### 10.1 tone() は音量フェードできない

現在の実装では音量スイープ処理を削除しています。`tone()` には音量指定がないためです。
