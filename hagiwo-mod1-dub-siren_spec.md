...
| D3 | `D3` | LED回路。`R10 10k` とLED `D4` を経由してGND | LFO状態LED `PinConfig::LED` |

`POT1`、`POT2`、`POT3` はいずれも `100k` のポテンショメータで、両端が `+5V` と `GND`、ワイパーが各Arduinoアナログ入力へ接続されています。

`F1` から `F4` は `AudioJack2_SwitchT` として描かれており、S端子はGNDに接続されています。T端子が信号線で、各ジャックの信号は抵抗、プルダウン、クランプダイオード、コンデンサを経由してArduinoピンへ入っています。現行スケッチでは `F4` / `D11` のみを音声出力として使用し、`F1`、`F2`、`F3` は使用していません。
...
`readPotValues()` は3つのPOTを読み、前回平滑化値との平均を取って制御値へ変換します。

- 95行目: 関数開始。
- 97-99行目: 前回値を `static` 変数として保持。
- 101行目: PITCH POT を読み、前回平滑化値と平均化。
- 102行目: PITCH の前回値を更新。
- 104行目: AMP POT を読み、前回平滑化値と平均化。
- 105行目: AMP の前回値を更新。
- 107行目: SPEED POT を読み、前回平滑化値と平均化。
- 108行目: SPEED の前回値を更新。
