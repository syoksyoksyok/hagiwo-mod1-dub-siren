 ...
 * - Added optional POT value smoothing (commented out, enable if needed).
 * - Improved comments for readability and maintenance.
 *
 * Fixes (2025-09-16):
 ...
 *
 * Changes (2025-09-16):
 * - Swapped RANDOM_HOLD and SINE in Waveform enum to change order: Random -> Square -> Sawtooth -> Reverse Sawtooth -> Sine.
 */
...
// PotValues struct for reading POT values (moved before readPotValues)
struct PotValues {
  float baseFreq;
  float amplitude;
  float step;
...
// Read and cache POT values (optional smoothing)
PotValues readPotValues() {
  static int lastSpeedValue = 0;
  int speedValue = analogRead(PinConfig::SPEED_POT); // Fixed: Was SPEAKER
  // Optional smoothing: Uncomment for 2-sample moving average to reduce POT noise
...
void setup() {
  ...
  randomSeed(analogRead(A3));  // Seed random with unused analog pin
...
void updateNormalOperation(float baseFreq, float amplitude, float step) {
  ...
  if (state.waveformType == RANDOM_HOLD && millis() - state.randomLastUpdate >= state.randomHoldTime) {
    ...
    state.randomValue = (random(-AudioConfig::AMP_MAX, AudioConfig::AMP_MAX + 1) / static_cast<float>(AudioConfig::AMP_MAX)) * amplitude;
  }
